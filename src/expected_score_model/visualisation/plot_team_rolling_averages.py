...
def create_team_rolling(shots, team, window=10, metric='xscore'):
    ...
    team_shots_for = team_shots.groupby(['Year', 'Round'])[[metric]].sum().rename(columns={metric:'for'})

    opp_shots = shots[((shots['Home_Team'] == team) | (shots['Away_Team'] == team)) & (shots['Team'] != team)]
    team_shots_against = opp_shots.groupby(['Year', 'Round'])[[metric]].sum().rename(columns={metric:'against'})

    team_rolling = team_shots_for.merge(team_shots_against, left_index=True, right_index = True)
...
def plot_all_team_rolling_figure(shots, window, metric = 'xscore', annotate = True, add_title=True, years = None):
    ...
    for plot_counter, (row, col) in enumerate(itertools.product(range(nrows), range(ncols))):
        ...
        team_rolling = create_team_rolling(shots, team, window, metric)
        ax = plot_team_rolling_ax(ax, team, team_rolling, annotate = annotate, years = years)
        
    if add_title:
        fig_text(
            x=0.13, y=0.91,
            s = "AFL - xscore for & against - 10-game rolling average.",
            size = 22,
            font = "Karla"
        )

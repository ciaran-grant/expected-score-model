...
class ModelEvaluator():
    ...
    def plot_ave(self):
        ...
        # Plot actual vs. predicted values
        ax.scatter(self.actual, self.expected)
        ax.plot([0, max(self.actual)], [0, max(self.actual)], 'r--')
        ax.set_xlabel(self.actual_name)
        ax.set_ylabel(self.expected_name)
    ...
    def plot_distribution(self):
        ...
        # Plot actual vs. predicted values
        sns.kdeplot(self.actual, shade=True, color="r", ax=ax)
        sns.kdeplot(self.expected, shade=True, color="b", ax=ax)
        ax.set_xlabel(self.actual_name)
    ...
    def _get_feature_plot_data(self, feature):
        ...
        if is_numeric_dtype(plot_data['feature']) & (len(np.unique(plot_data['feature'])) > 50):
            ...
            plot_data['feature'] = pd.qcut(plot_data['feature'], q = bins, labels = labels, duplicates = 'drop')
